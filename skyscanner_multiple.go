...
const baseUrlMultiple = `https://www.skyscanner.net/transport/d/%s/%s/%s/%s/%s/%s?currency=%s&adults=1&children=0&adultsv2=1&childrenv2=&infants=0&cabinclass=economy&ref=home#results`

//https://www.skyscanner.net/transport/d/mdea/2018-08-03/scla/scla/2018-08-03/syda?adults=1&children=0&adultsv2=1&childrenv2=&infants=0&cabinclass=economy&ref=home#results
//                     https://www.espanol.skyscanner.com/transporte/d/mdea/2018-09-20/scla/scla/2018-09-22/syda?adults=1&children=0&adultsv2=1&childrenv2=&infants=0&cabinclass=economy&ref=home#results

func InitCaptureMultiple(logpath string, initdate string, enddate string, origin string, mid string, dest string) {
	var err error
	var cycle int
	...
	for initdate != enddate {
		cycle++
		var url = getNewUrlMultiple(&initdate, origin, dest)
		err = c.Run(ctxt, skyscannerSearchMultiple(url, &best, &cheaper, &fastest))
		if err != nil {
			log.Fatal(err)
		}
...
func skyscannerSearchMultiple(url string, best *string, cheapest *string, fastest *string) chromedp.Tasks {
...
func getNewUrlMultiple(basenum *string, origin string, dest string) string {

	t, _ := time.Parse(longForm, *basenum)
	t = t.Add(time.Hour * 24)
	*basenum = t.Format(longForm)
	t = t.Add(time.Hour * 24)
	nextday := t.Format(longForm)
	//url := fmt.Sprintf(baseUrlOneWay, origin, dest, *basenum, currency, adults, children, adults, cabinclass)
	url := fmt.Sprintf(baseUrlMultiple, origin, *basenum, mid, mid, nextday, dest, currency)
